...
2. **APIs & Services → OAuth consent screen**. User type **External**. App name **Signal1**. Keep publishing status **Testing**. Add every operator Google account under **Test users**.
3. **Credentials → Create credentials → OAuth client ID → Web application**.
4. Authorized JavaScript origins:
   - `https://YOUR_DOMAIN`
5. Authorized redirect URIs:
   - `https://YOUR_DOMAIN/api/auth/callback/google`
6. Copy the client ID and secret into `.env` as `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET`.

Leave the app in Testing. Production verification is unnecessary for a private desk and would expose the consent screen more widely. Each new operator must be **both** a Google test user **and** on the Signal1 allowlist (env or Settings → Team).

Local development can add `http://127.0.0.1:3847` as an origin and `http://127.0.0.1:3847/api/auth/callback/google` as a redirect. Do not reuse the production client for a public laptop if you can avoid it — a second OAuth client named “Signal1 local” is cleaner.
...
**Set `AUTH_ALLOWED_EMAILS` before the box is reachable on 443.** If the allowlist is empty, the first Google login becomes admin. With the list set, even the first user must be on it, then becomes admin. Later operators are invited in **Settings → Team**.
...
- Settings → Team lists you as admin
...
## 7. Operators after go-live

1. Add their Gmail (or Google Workspace) address as a Google OAuth **test user**.
2. Settings → Team → **Invite Google account**.
3. They open `https://YOUR_DOMAIN` and use **Continue with Google**.

Revoke in Team to drop access. You cannot revoke the last admin.
...
| Google `AccessDenied` | Email not on Settings → Team / `AUTH_ALLOWED_EMAILS` |
